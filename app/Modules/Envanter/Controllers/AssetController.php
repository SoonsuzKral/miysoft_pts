...
class AssetController extends Controller
{
    ...
    /** Zimmet sözleşmesi PDF */
    public function zimmetPdf(AssetAssignment $assetAssignment)
    {
        $this->authorize('asset.assign');

        $assignment = $assetAssignment->load(['asset.type', 'personel.department', 'assignedBy']);
    ...
    /** Benzersiz seri no üret */
    public function generateSerial(): JsonResponse
    {
        $companyId = auth()->user()->company_id;
        do {
            $serial = 'SN-' . $companyId . '-' . now()->format('Ymd') . '-' . strtoupper(substr(uniqid(), -4));
        } while (Asset::where('serial', $serial)->exists());
    ...
    public function generateBarcode(): JsonResponse
    {
        $companyId = auth()->user()->company_id;
        do {
            $barcode = 'BC-' . $companyId . '-' . now()->format('Ymd') . '-' . strtoupper(substr(uniqid(), -4));
