...
<div class="flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-bold text-gray-900">Resmi Tatil Yönetimi</h1>
        <p class="text-sm text-gray-500 mt-1">Yıllık resmi tatil takvimini yönetin.</p>
    </div>
    <button onclick="openAddModal()" class="inline-flex items-center gap-2 px-5 py-2.5 bg-[#02E0FB] hover:bg-[#00b8d9] text-gray-900 font-semibold rounded-xl shadow-md transition-all">
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        <div class="flex items-center gap-4">
            ...
            <select id="yearFilter" class="px-4 py-2 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-[#02E0FB] focus:border-transparent" onchange="loadHolidays()">
        ...
        <table id="holidaysTable" class="min-w-full divide-y divide-gray-100">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Tatil Adı</th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Tarih</th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Gün</th>
                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Tür</th>
...
function loadHolidays() {
    ...
    body.innerHTML = rows.map((r, i) => `
        <tr class="hover:bg-gray-50 transition-colors">
            ...
            <td class="px-6 py-4 text-sm text-gray-600">${r.day}</td>
            <td class="px-6 py-4"><span class="px-2.5 py-1 rounded-full text-xs font-semibold bg-[#02E0FB]/10 text-[#00b8d9]">${r.type}</span></td>
            <td class="px-6 py-4 text-right"><button class="text-red-400 hover:text-red-600 text-xs font-medium">Sil</button></td>
        </tr>
    `).join('');
}
